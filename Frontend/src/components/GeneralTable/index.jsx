...
const GeneralTable = ({
  ...
  const handleFilterChange = (key, value) => {
    setValue((prevValue) => ({
      ...prevValue,
      filters: { ...prevValue.filters, [key]: value },
    }));
  };
  ...
  const filteredData =
    Array.isArray(value?.data) && value.data?.length > 0
      ? value?.data.filter((row) =>
          Object.keys(value.filters).every(
            (key) =>
              !value.filters[key] ||
              row[key].toString().includes(value.filters[key])
          )
        )
      : [];
  const getValueFilter = (key) => {
    return [...new Set(value?.data.map((item) => item[key]))];
  };
  ...
  const handleRowClick = (index) => {
    if (value.expandedRow.includes(index)) {
      setValue((prevValue) => ({
        ...prevValue,
        expandedRow: prevValue.expandedRow.filter(
      ...
      setValue((prevValue) => ({
        ...prevValue,
        expandedRow: [...prevValue.expandedRow, index],
      }));
    }
  };
    ...
    <div className='py-10 my-auto px-5 flex flex-col max-h-screen'>
      <div className='flex justify-between mb-3'>
        <div className='max-w-md'>
          {placeholderSearch && (
            <div className='relative flex items-center w-full border border-gray-300 h-12 rounded-lg focus-within:shadow-lg transition-all duration-300 bg-white overflow-hidden '>
        ...
        <div className='flex gap-2'>
          ...
          {functionButton && (
            <button
              ...
              className='cursor-pointer bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white h-12 px-4 border border-blue-500 hover:border-transparent rounded'
            >
              {functionButton}
            </button>
          )}
          ...
          <thead className='sticky top-0 bg-gray-100 z-10'>
            <tr>
              {columns.map((column) => (
                <th
                  key={column.key}
                  className='bg-gray-100/80 py-2.5 border-t border-b border-gray-200'
                >
                  <div
                    className={`${
                      column.isFilterable
                  ...
                  >
                    <div
                      className={`${
                        column.isFilterable
                          ? "flex flex-col items-start max-w-fit"
                          : ""
                      }`}
                    >
