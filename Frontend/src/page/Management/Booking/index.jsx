...
const BookingManager = () => {
  ...
  const fetchBookingDetail = async (id) => {
    try {
      const result = await bookingService.getBookingDetailArray(id);
      setData(result);
    } catch (error) {
      toast.error("Lỗi khi tìm kiếm đặt phòng");
    }
  };

  // Debounce searchData: sau 500ms không đổi mới thì cập nhật debouncedSearch
  useEffect(() => {
  ...
  // Gọi API khi debouncedSearch hoặc ngày thay đổi
  useEffect(() => {
    if (debouncedSearch !== "") {
      fetchBookingDetail(debouncedSearch);
    } else {
      ...
      return (
          <div className="p-3">
                <div className="">
                  ...
                  <button type="button" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 cursor-pointer">Thanh toán</button>
                  </div>
                  ...
                  <div className="flex gap-30 text-base mb-4">
                      ...
                      <div><strong>Đã thanh toán:</strong> {(row.amount ?? 0).toLocaleString()} VND</div>
                  </div>
                  ...
                  <div className="flex justify-end mt-4">
                      <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-base w-1/4">
                          <div><strong>Tổng tiền:</strong></div>
                          <div className="text-right text-nowrap">{Number(row.total_price ?? 0).toLocaleString()} VND</div>

                          <div><strong>Đã thanh toán:</strong></div>
                          <div className="text-right text-nowrap">{Number(row.amount ?? 0).toLocaleString()} VND</div>

                          <div><strong>Còn lại:</strong></div>
                          <div className="text-right text-nowrap">{(Number(row.total_price ?? 0) - Number(row.amount ?? 0)).toLocaleString()} VND</div>
                      </div>
                  </div>
