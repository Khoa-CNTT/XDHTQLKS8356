                ...
                >
                  <div className='flex gap-10 items-center justify-between'>
                    <div className='flex gap-2 items-center mb-2'>
                      ...
                      <div className=''>
                        {`${order?.checkin.split("-")[2]}/${
                          order?.checkin.split("-")[1]
                    ...
                    <div className='flex gap-2 items-center mb-2'>
                      ...
                      <div className=''>
                        {`${order?.checkout.split("-")[2]}/${
                          order?.checkout.split("-")[1]
                        }/${order?.checkout.split("-")[0]}`}
                      </div>
                    </div>
                    <div className='flex gap-2 items-center mb-2'>
                      <div className='font-medium'>Tổng số ngày:</div>
                      <div className=''>{order?.total_day}</div>
                    </div>
                  ...
                  <div className=''>
                    ...
                    <div className='overflow-x-auto'>
                      <div className='relative w-full overflow-auto rounded-xl shadow bg-white'>
                        <>
                          <table className='min-w-full text-left table-auto text-sm text-slate-800 mb-5'>
                            <thead className='bg-blue-100/70 text-slate-500'>
                              <tr>
                                <th className='p-4 border-b border-slate-300 font-normal'>
                                  Tên Phòng
                                </th>
                                <th className='p-4 border-b border-slate-300 font-normal text-center'>
                                  Số Lượng
                                </th>
                                <th className='p-4 border-b border-slate-300 font-normal text-right'>
                                  Giá
                                </th>
                                <th className='p-4 border-b border-slate-300 font-normal text-right'>
                                  Tổng Giá
                            ...
                            <tbody>
                              {order.details.map((detail, detailIndex) => (
                                <tr
                                  key={`room-${detailIndex}`}
                                  className='hover:bg-blue-50 transition'
                                >
                                  <td className='p-4 border-b font-semibold border-slate-200'>
                                    {detail.room_name} (Số phòng:{" "}
                                    {detail.room_number})
                                  </td>
                                  <td className='p-4 border-b font-semibold border-slate-200 text-center'>
                                    1
                                  </td>
                              ...
                              {order.services && order.services.length > 0 ? (
                                order.services.map((service, serviceIndex) => (
                                  <tr
                                    ...
                                    className='hover:bg-slate-50 bg-white transition'
                                  >
                        ...
                        <div className='mt-4 flex justify-end gap-4'></div>
                      </div>
                    </div>
                  </div>
                </div>
